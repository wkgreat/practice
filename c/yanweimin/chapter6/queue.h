...
int initqueue(Pqueue queue)
{
	...
	queue->base = (QElemtype*)malloc(sizeof(Elemtype)*MAXSIZE);
...
int enqueue(Pqueue queue, QElemtype e)
{
	if((queue->tail+1) % MAXSIZE - queue->head == 0) {printf(" queue overflow!\n"); return 1;}
...
{
	if(queue->head == queue->tail) {printf("queue empty!\n"); return ERROR;}
	*pe = queue->base[queue->head];
	queue->head = (queue->head+1) % MAXSIZE;
	return OK;
}

int queueempty(Pqueue queue)
{
	if(queuelength(queue) == 0) return 1;
	return 0;
}
